            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <?php if ($_SESSION["level_user"] == 1) : ?>
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="./list.php">Permintaan</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="./list.php">Transaksi</a>
                        </li>
                        <li class="nav-item">
                    ...
                    <?php else : ?>
                        <li class="nav-item">
                            <a class="nav-link active" href="./listKendaraan.php">Kendaraan</a>
                        </li>
                        <li class="nav-item">
                    ...
                    <div class="card" style="width: 18rem;">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                ...
                                <span class="<?= $row["in_use"] == 0 ? "bg-success" : "bg-secondary" ?> fw-bold text-white px-2 py-1 rounded" style="font-size: 10px;">
                                    <?= $row["in_use"] == 0 ? "Available" : "Sedang Dipakai" ?>
                                </span>
                            </div>
                            <p class="card-text"><?= $row["brand"] ?> <?= $row["model"] ?> - <?= $row["type"] ?></p>
                            <p class="fw-bold">Rp. <?= $row["price"] ?></p>
                            <a href="<?= $row["in_use"] == 0 ? "../server/approval.php?vehicle_id=" . $row["vehicle_id"] : "" ?>" <?= $row["in_use"] == 0 ? "" : "style='cursor: default;'" ?>>
                                <button class="btn btn-sm btn-primary" <?= $row["in_use"] == 0 ? "" : "disabled" ?>>Pinjam</button>
                            </a>
                        </div>
                    </div>
                <?php } ?>
